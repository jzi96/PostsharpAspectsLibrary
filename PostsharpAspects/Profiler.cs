...
namespace Zieschang.Net.Projects.PostsharpAspects
{
    ...
    public sealed class Profiler : IDisposable
    {
        ...
        [ThreadStatic]
        private static List<Profiler> _hirarchieStorage = new List<Profiler>();
        //use a static instance to reduce overhead!'
        private static readonly Stopwatch Watch =  Stopwatch.StartNew();

#if(DEBUG)
        private readonly ILog _logger = LogManager.GetLogger(typeof(Profiler));
#else
        ...
        public Profiler(string name, string category, TraceLevel logLevel, bool hirachical)
        {
            ...
            if(hirachical)
            {
                if(_hirarchieStorage == null)
                    _hirarchieStorage = new List<Profiler>();
        ...
        private void Dispose(bool isDisposing)
        {
               ...
               && _startTime>0)
            {
                ...
                if(this.Hirachical
                   && _hirarchieStorage != null)
                {
                    //remove from stack
                    if(_hirarchieStorage[_hirarchieStorage.Count - 1]
                       == this)
